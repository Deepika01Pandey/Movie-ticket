...
const Drama = () => {
  ...
  return (
    <>
      <p className="text-2xl max-sm:text-lg font-sans font-bold pl-4 max-sm:pl-2 pb-2 mt-6 max-sm:mt-2">Drama</p>
      <div className="flex flex-wrap gap-2 font-sans justify-evenly items-center">
        {Drama.map((movies, index) => {
          return (
            <div
              key={index}
              className="w-[260px] my-2 bg-slate-300 max-sm:w-[140px] cursor-pointer rounded-lg shadow-lg mx-2 max-sm:mx-1"
            >
              <img
              ...
              <div className="p-2">
                ...
                <div className="flex max-sm:flex-col justify-between items-center mt-3 max-sm:mt-1">
                  <p className="text-red-600 font-medium text-base max-sm:text-sm">
                    <span className="text-black">IMDb: </span>{movies.imdb["rating"]["$numberDouble"]}
                  </p>
                  <button onClick={() => moveToBookingPage(movies['_id']['$oid'])} className="bg-black max-sm:mt-1 text-white font-bold px-3 py-2 max-sm:text-xs max-sm:px-2 max-sm:py-1 max-sm:text-normal text-sm text-medium rounded-lg hover:bg-red-600">
                    Book Now
                  </button>
                </div>
              </div>
